...
const config = JSON.parse(
  fs.readFileSync(path.join(__dirname, "..", "levels", "4.txt"), "utf8"),
);
...
function getRequiredExit(segIdx, compound, currentTime) {
  ...
  for (let j = segIdx + 1; j < segments.length; j++) {
    if (segments[j].type === "corner") {
      ...
      const mcs = maxCornerSpeed(segments[j].radius_m, safeFriction);
...
function simulateLap(entrySpeed, weatherTargets, compound, initialTime) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (seg.type === "straight") {
      ...
      const reqExit = getRequiredExit(i, compound, initialTime + time);
...
for (let lap = 1; lap <= TOTAL_LAPS; lap++) {
  // Use the engineered segment-by-segment simulation loop
